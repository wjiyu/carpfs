...
func ShuffleChunks(chunkMaps map[Ino][]string, size int) ([]string, error) {
	var chunkIds []Ino
	for key := range chunkMaps {
		chunkIds = append(chunkIds, key)
	}

	shuffleIds := groupChunkIds(shuffle(chunkIds).([]Ino), size)
...
func groupChunkIds(slice []Ino, size int) [][]Ino {
	if slice == nil {
		return nil
	}
	if size <= 0 {
